...
import {ILendingPool} from './ILendingPool_gateway.sol';
...
contract WETHGateway is IWETHGateway, Ownable {
  ...
  function withdrawETH(uint256 amount, address to) external override {
    ...
    uint256 fee =  amountToWithdraw.mul(uint256(POOL.withdrawFee())).div(10000);
    xWETH.transferFrom(msg.sender, address(this), amountToWithdraw);
    POOL.withdraw(address(WETH), amountToWithdraw, address(this));
    WETH.withdraw(amountToWithdraw.sub(fee));
    _safeTransferETH(to, amountToWithdraw.sub(fee));
  }
  ...
  function borrowETH(
    ...
    uint256 fee =  amount.mul(uint256(POOL.borrowFee())).div(10000);
    POOL.borrow(address(WETH), amount, msg.sender);
    WETH.withdraw(amount.sub(fee));
    _safeTransferETH(msg.sender, amount.sub(fee));
  }
