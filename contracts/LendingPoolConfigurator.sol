 ...
 * @author OKLend
 * @dev Implements the configuration methods for the OKLend protocol
 **/
...
contract LendingPoolConfigurator is VersionedInitializable {
  ...
  function initReserve(
    ...
    require(
      ...
      Errors.LPC_INVALID_XTOKEN_POOL_ADDRESS
    );
    require(
  ...
  function setCollateralManager(address _collateralManager)
    ...
    onlyPoolAdmin
  {
    pool.setCollateralManager(_collateralManager);
    emit CollateralManagerChanged(_collateralManager);
  }
  
  function setBorrowFee(uint8 _fee)
    external
    onlyPoolAdmin
  {
    pool.setBorrowFee(_fee);
    emit BorrowFeeChanged(_fee);
  }
  
  function setWithdrawFee(uint8 _fee)
    external
    onlyPoolAdmin
  {
    pool.setWithdrawFee(_fee);
