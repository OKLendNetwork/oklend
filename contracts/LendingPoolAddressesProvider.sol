 ...
 * - Owned by the OKLend Governance
 * @author OKLend
 **/
contract LendingPoolAddressesProvider is Ownable, ILendingPoolAddressesProvider {
  ...
  bytes32 private constant OKLEND_TOKEN = 'OKLEND_TOKEN';
   ...
   **/
  function setLendingPoolImpl(address pool,address _weth) external override onlyOwner {
    _updatePoolImpl(LENDING_POOL, pool, _weth);
  ...
  function getOklendToken() external view override returns (address) {
    return getAddress(OKLEND_TOKEN);
  }

  function setOklendToken(address okl) external override onlyOwner {
    _addresses[OKLEND_TOKEN] = okl;
    emit OklendTokenUpdated(okl);
  }
   ...
   * @param newAddress The address of the new implementation
   **/
  function _updatePoolImpl(bytes32 id, address newAddress,address _weth) internal {
    address payable proxyAddress = payable(_addresses[id]);

    InitializableImmutableAdminUpgradeabilityProxy proxy =
      InitializableImmutableAdminUpgradeabilityProxy(proxyAddress);
    bytes memory params = abi.encodeWithSignature('initialize(address,address)', address(this), _weth);
