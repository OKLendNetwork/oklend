...
contract VariableDebtToken is DebtTokenBase, IVariableDebtToken, ReentrancyGuard {
    ...
    {
      rewardsDistribution = ILendingPoolAddressesProvider(_addressesProvider).getRewardsDistribution();
    ...
    function mint(
        ...
        uint256 amount,
        uint256 index
    ) external override onlyLendingPool updateReward(onBehalfOf) returns (bool) {
        if (user != onBehalfOf) {
